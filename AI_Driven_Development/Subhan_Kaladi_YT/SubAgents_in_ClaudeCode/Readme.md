...
- [YouTube Link](https://www.youtube.com/watch?v=r-wvRH2WC70&list=WL&index=5)
- [Blog Link](https://yt-blogs.subhankaladi.xyz/blog/sub-agents-in-claude-code)
...
- **Tools available:** Glob - File pattern matching
...
**How it works:** When you're in plan mode and Claude needs to understand your codebase to create a plan, it delegates research tasks to the Plan subagent. This prevents infinite nesting of agents while still allowing Claude to gather necessary context.
...
[Internally invokes Plan subagent to explore auth-related files][Plan subagent searches codebase and returns findings]
...
- **Available subagents** - Which agents match?
- **Agent descriptions** - Which is most appropriate?
- **Tool requirements** - Does the agent have needed tools?
- **Context needs** - Does it benefit from a clean context?
...
- Describe your subagent to Claude - let it generate the initial version
- Customize the generated agent to make it yours
- Select tools from the comprehensive list
...
model: sonnet
---
You are a test automation expert. When you see code changes, proactively run the appropriate tests. If tests fail, analyze the failures and fix them while preserving the original test intent.EOF
```
...
| `description`   | ✅ Yes   | Natural language description of purpose (used by Claude to decide when to use it) |
| `tools`         | ❌ No    | Comma-separated list; if omitted, inherits **all tools** from main thread   |
| `model`         | ❌ No    | Model alias (`sonnet`, `opus`, `haiku`) or `inherit`; default is subagent model |
...
- **sonnet** — Most capable and balanced *(default for subagents)*
- **opus** — Most powerful, best for complex reasoning
- **haiku** — Fastest and most cost-effective
- **inherit** — Uses the same model as the main conversation

**Example use cases:**

- `model: haiku:` Fast search or lightweight lookup agent
- `model: sonnet:`Code review and general-purpose development agent
- `model: opus:` Complex architecture planning and deep reasoning tasks
- `model: inherit:` Matches the main conversation’s model and capabilities
#### Available Tools

Subagents can access any of **Claude Code’s internal tools**. You can configure tool access in the following ways:

1. Omit the `tools` Field *(Recommended)* - Inherits **all tools** from the main thread, including:
- **Built-in tools**: `Read`, `Write`, `Edit`, `Bash`, `Grep`, `Glob`, etc.
- **Connected MCP server tools**

2. **Specify Individual Tools** *(Granular Control)*
- Limit the subagent to only specific tools:
```md
    tools: Read, Grep, Glob, Bash
```
**Common tools**:
...
--- 
<!--
This agent manages deployments and environment configurations. It has access to deployment scripts and environment files. Always runs pre-deployment checks before deploying.
--> 
...
> Have the security-scanner check the code, then use code-reviewer for quality, and finally test-runner to verify
```
**How it works**:
...
id: unique-id 
title: Page Title 
sidebar_label: Short Label sidebar_position: 1 
...
```YAML
--- name: content-reviewer description: Reviews Docusaurus documentation for clarity, accuracy, and consistency. Use after writing or updating documentation to ensure quality. tools: Read, Grep, Glob, Bash model: sonnet 
--- 
